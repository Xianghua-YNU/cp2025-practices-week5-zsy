...
def generate_coin_sequence(n_flips, p_head=0.08):
    ...
    return np.random.choice([1, 0], size=n_flips, p=[0.08, 0.92])
...
def calculate_waiting_times(coin_sequence):
    ...
    positive_indices = np.nonzero(coin_sequence)[0]
    # 2. 使用np.diff计算连续两个正面之间的间隔
    waiting_times = np.diff(positive_indices)
    # 3. 减1得到中间的反面数量
...
def plot_waiting_time_histogram(waiting_times, log_scale=False, n_flips=None):
    ...
    if n_flips is not None:
        plt.title(f'Histogram of Waiting Times ({n_flips} Coin Flips)')
    else:
...
def analyze_waiting_time(waiting_times):
    ...
    theoretical_mean = 1 / (1 - 0.08)
    # 3. 返回包含这些统计量的字典
    exponential_mean = -1 / np.log(1 - 0.08)
    return {
        "mean": mean,
        "std": std,
